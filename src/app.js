...
function SkladLedger() {
  ...
  const [fbsWarehouses, setFbsWarehouses] = useState([]); // склады продавца (WB)
  const [fbsWarehouse, setFbsWarehouse] = useState(() => { try { return Number(localStorage.getItem('fbs_wh')) || 0; } catch (_) { return 0; } });
  function chooseFbsWarehouse(id) {
    const n = Number(id) || 0;
    setFbsWarehouse(n);
  ...
  async function syncFbsOrders() {
    ...
    try {
      ...
      // Подтягиваем склады продавца (для фильтра «свой склад»).
      try {
        const wr = await fetch(`${WB_PROXY_URL}/fbs/warehouses`);
        const wd = await wr.json();
        if (Array.isArray(wd)) setFbsWarehouses(wd);
      } catch (_) {}
    } catch (e) {
      console.error(e);
    ...
    /*#__PURE__*/React.createElement("div", { style: { display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.6fr 1fr', gap: 13, marginBottom: 13 } },
      /*#__PURE__*/React.createElement("div", { className: "skl-card" },
        ...
        dashChart.empty
          ...
          : /*#__PURE__*/React.createElement("svg", { viewBox: "0 0 330 120", preserveAspectRatio: "none", style: { width: '100%', height: 210, display: 'block' } },
